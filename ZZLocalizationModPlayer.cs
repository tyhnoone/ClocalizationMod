		...
		public override void OnEnterWorld(Player player)
        {
			if (LanguageManager.Instance.ActiveCulture == GameCulture.Chinese)
            {
                ...
                if (CalamityMod != null)
                {
                    ...
                    {Main.NewText("检测到你已启用CalamityMod，当前汉化对应版本是" + Cala_vers +"，增加部分灾厄文本汉化", 255, 255, 147, false);}
                    else
                    {Main.NewText("检测到你已启用CalamityMod v" + CalamityMod.Version + "，当前汉化对应版本是v" + Cala_vers, 255, 255, 147, false);}

                    Main.NewText("由于Calamity的更新频率，ZZLocalizationMod将不完全保证汉化文本的正确性", 26, 255, 138, false);
                }
            }
