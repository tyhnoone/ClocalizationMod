			...
			for (int i = 0; i < 8; i++)
				{
					...
					if ((Main.npcChatText == null || Main.npcChatText == "") && Main.player[Main.myPlayer].sign < 0)
					{
						...
						if(i==7 && !flag7)
						{
							...
							for (int l = 0; l < 200; l++)
							{
								if (Main.npc[l].active && !Main.npc[l].townNPC && Main.npc[l].life >= 5)
								{
									NPC type = Main.npc[l];
									if ((Main.npc[l].Center - Main.player[Main.myPlayer].Center).Length() < (float)num15)
									{
										...
										if(npcaround != null)
										{
											if ((Main.npc[l].Center - Main.player[Main.myPlayer].Center).Length() <= (npcaround.Center - Main.player[Main.myPlayer].Center).Length())
											{
												npcaround = type;
											}
										}
